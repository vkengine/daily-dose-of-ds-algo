...
def validIPAddresses(string):
    ...
    for i in range(1, min(len(string), 4)):
        ...
        if not is_valid(current_ip_address_parts[0]):
            continue
        ...
        for j in range(i + 1, i + min(len(string) - 1, 4)):
            ...
            if not is_valid(current_ip_address_parts[1]):
                continue
            ...
            for k in range(j + 1, j + min(len(string) - j, 4)):
                ...
                if is_valid(current_ip_address_parts[2]) and is_valid(current_ip_address_parts[3]):
                    ip_addressed_found.append('.'.join(current_ip_address_parts))

    return ip_addressed_found
